...
namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ
{
    ...
    /// BOM 替代料
    /// </summary>
    public class Bom_opcomponentsub : BizBase
    { 
        ...
        public Bom_opcomponentsub(ref ADODB.Connection conn, ref System.Data.DataSet ds, string ufConnStr,string opertype)
            : base(conn, ufConnStr)
        {


            oracleTableName = "MES_CQ_Bom_opcomponentsub";   //目标表名
            oraclePriKey = "bomid";      //目标表逻辑主键 
            fieldcmpTablename = "MES_CQ_bom_bom";
            ufTableName = "Bom_opcomponentsub"; // "SaleOrderQ";       //来源表名
        ...
        public Bom_opcomponentsub(ref ADODB.Connection conn, int bomId, string ufConnStr, string opertype)
            : base(conn, ufConnStr)
        {
            oracleTableName = "MES_CQ_Bom_opcomponentsub";   //目标表名
            oraclePriKey = "bomid";      //目标表逻辑主键 
        ...
        public override StringBuilder CreateDeleteString()
        {
            ...
            {
                string sql = " DELETE FROM " + oracleTableName + " WHERE  " + oracleTableName + ".did in ( select MES_CQ_Bom_opcomponent.did from MES_CQ_Bom_opcomponent where MES_CQ_Bom_opcomponent.id in( select id from MES_CQ_bom_bom with(nolock) where  bomid ='" + bomId + "')  )";
