...
namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ
{
    ...
    /// 组装单
    /// </summary>
    public class AssemVouch : U8.Interface.Bus.Event.SyncAdapter.Biz.BizBase
    {

        private string _cAVCode; 
        AssemVouchs detailBiz;

        public AssemVouch(ref ADODB.Connection conn, IXMLDOMDocument2 doc, IXMLDOMDocument2 docbody, string ufConnStr, string _opertype)
            : base(conn, ufConnStr)
        {

            oracleTableName = "MES_CQ_AssemVouch";   //目标表名
            oraclePriKey = "cAVCode";      //目标表逻辑主键 
            fieldcmpTablename = "MES_CQ_AssemVouch";
        ...
        private void SetData(string cAVCode)
        {
            ...
            detailBiz = new AssemVouchs(ref conn, cAVCode, ufConnStr, opertype);
        ...
        public override DataTable GetSourceData(string sourceTableName, string sourceKeyName, string sourceKeyValue, string colNames)
        {
            ...
            string sql = "SELECT " + colNames + " FROM " + ufTableName + "  WHERE cVouchType = '13' AND cAVCode ='" + _cAVCode + "' ";
