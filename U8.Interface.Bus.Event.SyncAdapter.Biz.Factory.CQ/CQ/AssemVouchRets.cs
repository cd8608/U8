...
namespace U8.Interface.Bus.Event.SyncAdapter.Biz.Factory.CQ
{
    ...
    public class AssemVouchRets : U8.Interface.Bus.Event.SyncAdapter.Biz.BizBase
    {

        private string _cAVCode;

        public AssemVouchRets(ref ADODB.Connection conn, string cAVCode, string ufConnStr, string _opertype)
            : base(conn, ufConnStr)
        {

            oracleTableName = "MES_CQ_AssemVouchRets";   //目标表名
            oraclePriKey = "cAVCode";      //目标表主键 
            fieldcmpTablename = "MES_CQ_AssemVouchRets";
        ...
        public override DataTable GetSourceData(string sourceTableName, string sourceKeyName, string sourceKeyValue, string colNames)
        {
            ...
            string sql = "SELECT " + colNames + " FROM " + ufTableName + " WHERE ID IN ( SELECT ID FROM AssemVouch WITH(NOLOCK) WHERE cVouchType = '14'  AND cAVCode ='" + _cAVCode + "'  ) ";
        ...
        public override StringBuilder CreateDeleteString()
        {
            ...
            {
                string sql = " DELETE FROM " + oracleTableName + " WHERE  " + oracleTableName + ".cAVDID in( select  autoid from " + ufTableName + " sb with(nolock) INNER JOIN AssemVouch sh WITH(NOLOCK) ON sh.id = sb.id where sh.cVouchType = '14' AND sh.cAVCode ='" + _cAVCode + "') ";
                //string sql = " DELETE FROM " + oracleTableName + " WHERE  " + oracleTableName + ".cAVDID in( select  autoid from " + ufTableName + " sb with(nolock) where cAVCode ='" + _cAVCode + "') ";
